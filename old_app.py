...
def upload_file():
    ...
    if filename.endswith('.zip'):
        ...
        for series in os.listdir(dicom_path):
            if not series.startswith('L-SPINE'):
                shutil.rmtree(os.path.join(dicom_path, series))
            else:
                for dicom in os.listdir(os.path.join(dicom_path, series)):
                    if not dicom.startswith('T2'):
                        shutil.rmtree(os.path.join(dicom_path, series, dicom))

        sag_paths = []
        axi_paths = []

        for ser in os.listdir(dicom_path):
            os.makedirs(f'{base_dir}/{id}/{ser}/sagittal', exist_ok=True)
            os.makedirs(f'{base_dir}/{id}/{ser}/axial', exist_ok=True)
            for view in os.listdir(os.path.join(dicom_path, ser)):
                if str(view).startswith('T2_TSE_SAG'):
                    sag_paths.extend([os.path.join(dicom_path, ser, view, file) for file in os.listdir(os.path.join(dicom_path, ser, view))])
                elif str(view).startswith('T2_TSE_TRA'):
                    axi_paths.extend([os.path.join(dicom_path, ser, view, file) for file in os.listdir(os.path.join(dicom_path, ser, view))])

        for file_name in sag_paths:
            ...
            series_path = file_name.split("/")[-3]
            file_name = file_name.split("/")[-1]
            output_path = os.path.join(
                dicom_path,
                f'{series_path}/sagittal/{file_name.replace(".ima", ".jpg")}'
            )
            image = Image.fromarray(image_array)
            image.save(output_path)

        for file_name in axi_paths:
            dicom_image = pydicom.dcmread(file_name)
            image_array = dicom_image.pixel_array
            ...
            output_path = os.path.join(
                dicom_path,
                f'{series_path}/axial/{file_name.replace(".ima", ".jpg")}'
            )
            image = Image.fromarray(image_array)
            image.save(output_path)
